...
// Graphic look
double z_bg_cutoff = 3.0;
double z_gr_cutoff = 6.0;
...
////////////////
void usage();
int read_data(vector<halo> *v, ifstream *f);
...
// Calculate a color based on the halo age
void calculate_color(GLdouble *color, double z_form) {
	// If redshift is negative, then something is wrong
	if (z_form < 0.0) {
	...
	if (z_form < z_bg_cutoff) {
		color[0] = 0.0;
		color[1] = z_form/z_bg_cutoff;
		color[2] = 1.0;
	}
	else if (z_form < z_gr_cutoff) {
		color[0] = (z_form-z_bg_cutoff)/(z_gr_cutoff-z_bg_cutoff);
		color[1] = 1.0;
		color[2] = 1.0 - (z_form-z_bg_cutoff)/(z_gr_cutoff-z_bg_cutoff);
	}
	else {
		color[0] = 1.0; 
		color[1] = exp(-(z_form-z_gr_cutoff)/z_gr_cutoff);
		color[2] = 0.0;
	}
#else
	color[0] = exp(-z_r_factor*z_form);
	color[1] = exp(-z_g_factor*z_form);
	color[2] = exp(-z_b_factor*z_form);
#endif
}
...
void render(SDL_Surface *screen, vector<halo> *halos, camera *cam) {
	...
	// Draw redshift scale, first the colors, then texts.
	...
	for (int i=0; i < 21; i++) {
		y = 200 + i*h;
		calculate_color(color, (double)(i/2.0));
	...
	for (int i=0; i < 21; i++) {
		y = 200 + i*h;
		snprintf(str, max_strlen, "z = %2.2f", i/2.0);
...
int main(int argc, char **argv) {
	...
	box_size = strtod(argv[1], NULL);
	if (box_size < 0.1) {
		usage();
		return 0;
	}
	printf("Box size is %f megaparsecs\n", box_size);
	cam.r = box_size;
...
// Read halo data from a halodump file to a given vector
// XXX: We assume here that the halos are unique, ie. no main halo is also
// a subhalo, and that no two main halos share a subhalo
int read_data(vector<halo> *v, ifstream *f) {
	return parse_halodump_file(v, f);
}
...
void draw_halos(vector<halo> *halos) {
	...
	for (unsigned int i=0; i < halos->size(); i++) {
		...
		calculate_color(color, h.z_form);
